        ...
        it('should register successfully with status code 201', function (done) {
            ...
            chai
                ...
                .then((result) => {
                    ...
                    expect(result.body).to.have.keys(['__v', '_id', 'balance', 'email', 'imageUrl', 'name', 'password', 'role'])
        ...
        it('should failed to register an admin(wrong admin password) with status code 400', function (done) {
            ...
            chai
                ...
                .then((result) => {
                    ...
                    expect(result).to.have.status(400)
                    expect(result.body).to.be.an('object')
                    expect(result.body.message).to.equal('Incorrect password for register as admin')
                    done()
                }).catch((err) => {
                    console.log(err)
                    done()
        ...
        it('should get users successfully with status code 200', function (done) {
            chai
                ...
                .then((result) => {
                    // console.log(result.body)
                    expect(result).to.have.status(200)
                    expect(result.body).to.have.keys(['_id','balance', 'email', 'imageUrl', 'name', 'role'])
                    done()
                }).catch((err) => {
                    console.log(err);
                    done()
